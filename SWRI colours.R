setwd("C:/Users/hf/Desktop/R projects")
...
SWRI_palettes <- list(
  ...
  `bold` = SWRI_colours("SWRI pink", "orange", "yellow", "lime", "teal", "SWRI purple"),
...
ggplot(iris, aes(Sepal.Width, Sepal.Length, color = Species)) +
  geom_point(size = 4) +
  scale_colour_SWRI(palette = "new")

ggplot(iris, aes(Sepal.Width, Sepal.Length, color = Sepal.Length)) +
  geom_point(size = 4, alpha = .6) +
  scale_colour_SWRI(discrete = FALSE, palette = "new")

ggplot(mpg, aes(manufacturer, fill = manufacturer)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_SWRI(palette = "spring", reverse = TRUE, guide = "none")
...
df2 <- data.frame(x = seq(1,500),
                  y = rep("a",500))
df3 <- data.frame(x = seq(1, 4),
                  y = rep("A", 4))

ggplot(df2, aes(x = factor(x), y = factor(y), fill = x)) +
  geom_col(width = 1) +
  #scale_fill_identity() +
  theme_void() +
  theme(legend.position = "none") +
  scale_fill_SWRI(palette = "bold", discrete = FALSE, reverse = FALSE, guide = "none")

#library(pheatmap)
#set.seed(123)
#data_matrix <- matrix(rnorm(100), nrow = 10, ncol = 10)

#pheatmap(data_matrix, color = SWRI_pal(palette = "warm")(40))
...
#my_palette <- SWRI_pal(palette = "bold")(100)
#my_colour_function <- colorRamp2(c(-2, 0, 2), my_palette[c(1, 50, 100)])

#my_colour_function(c(-2, -1, 0, 1, 2))
#"#F4546CFF" "#DC9068FF" "#BABD63FF" "#7D756CFF" "#34346CFF"#

#my_colour_function(c(-100, -5, 0, 5, 100))
#"#F4546CFF" "#F4546CFF" "#BABD63FF" "#34346CFF" "#34346CFF"
